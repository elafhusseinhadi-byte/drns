...
def init_uavs(n: int, db: Session = Depends(get_db)):
    ...
    # current UAV count
    existing = db.query(UAVState).count()

    # Case 1: Add missing UAVs
    if existing < n:
        start = existing + 1
    ...
    # Case 2: Remove excess UAVs
    if existing > n:
        db.query(UAVState).filter(UAVState.uav_id > n).delete()
        db.commit()
...
def compute_velocities(db: Session, history_window: int = 3):
    ...
    for row in db.query(UAVState).all():
        ...
        if len(hist) < 2:
            result[row.uav_id] = {"vx": 0.0, "vy": 0.0}
            continue
...
def compute_conflicts(uavs):
    ...
    for i in range(len(uavs)):
        ...
        for j in range(i + 1, len(uavs)):
            ...
            # Determine status by distance priority
...
def put_uav(uav: UAVIn, db: Session = Depends(get_db)):
    ...
    if state is None:
        raise HTTPException(
            status_code=400,
            detail=f"uav_id {uav.uav_id} not initialized. Call /init?n=number first."
        )
